...
// To register the swagger generator
builder.Services.AddSwaggerGenSetup(SwaggerModuleDoc.BoscoAFHAuthDocs);
...
app.UseCommonAppSetup(SwaggerModuleDoc.BoscoAFHAuthDocs, app.Services.GetRequiredService<IOptions<SwaggerGenOptions>>().Value);
...
app.MapScalarForSwashbuckle(SwaggerModuleDoc.BoscoAFHAuthDocs, SwaggerModuleDoc.BoscoAFHAuth);

app.MapGet("/", () => Results.Text(DefaultData.WebStartPage.Replace("{0}", SwaggerModuleDoc.BoscoAFHAuth), "text/html")).ExcludeFromDescription(); // Exclude this endpoint from Swagger
