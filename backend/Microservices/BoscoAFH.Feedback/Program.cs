...
// To register the swagger generator
builder.Services.AddSwaggerGenSetup(SwaggerModuleDoc.BoscoAFHFeedbackDocs);
...
app.UseCommonAppSetup(SwaggerModuleDoc.BoscoAFHFeedbackDocs, app.Services.GetRequiredService<IOptions<SwaggerGenOptions>>().Value);
...
app.MapScalarForSwashbuckle(SwaggerModuleDoc.BoscoAFHFeedbackDocs, SwaggerModuleDoc.BoscoAFHFeedback);

app.MapGet("/", () => Results.Text(DefaultData.WebStartPage.Replace("{0}", SwaggerModuleDoc.BoscoAFHFeedback), "text/html")).ExcludeFromDescription(); // Exclude this endpoint from Swagger
